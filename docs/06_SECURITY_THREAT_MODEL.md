...
## 29.1 Phase 12 Deployment Threat Contract

Phase 12 adds no application capability. It moves the already-tested system
through external runtime boundaries, so the following deployment threats are
release blocking:
...
| Audit/evidence bypass | Release result records SHA, digests, index, smoke, worker, monitor, and rollback outcomes without sensitive payloads | Bounded machine-readable release evidence and log scan |

The current deployment is blocked while the Phase 9 encryption selectors and
dedicated smoke credentials are absent. Missing deployment secrets must never
be replaced with generated runtime defaults. The deep-stopped ECS state is not
a healthy rollback environment until reconstructed and smoke-tested.
